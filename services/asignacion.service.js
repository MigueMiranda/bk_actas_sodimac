...
class AsignacionService {
  ...
  for (const a of activos) {
    await models.Movimiento.create({
      ...
      caso: "pendiente",
      acta: "pendiente",
      estadoElemento: ubicacion.estados,
      tiendaId: a.tienda,
  ...
  const estadoEsAsignado = ubicacionData?.estados === "Asignado";
  ...
  if (respuesta === "aprobado") {
    ...
    if (notificacion) {
      ...
      const rta = await service.mailOptions(mailOptions, "confirmacion");
      return rta;
    }
  ...
  // === CASO RECHAZADO ===
    ...
    if (notificacion) {
      ...
      return await service.mailOptions(mailOptions, "rechazado");
    }
  ...
  async generarActaPDF(contacto, activo, ubicacion, idAsignacion) {
    const templateHtml = fs.readFileSync(
      path.join(__dirname, "../templates/acta.html"),
      "utf8"
    );
    const template = handlebars.compile(templateHtml);

    const html = template({
      usuario: contacto.name,
      fecha: new Date().toLocaleDateString(),
      elementos: [
        {
          serial: activo.serial,
          descripcion: activo.tipo || "Sin descripción",
          placa: activo.placa,
        },
      ],
    ...
    const outputPath = path.join(
      __dirname,
      `../public/actas/acta_${idAsignacion}.pdf`
    );
    await page.pdf({
      path: outputPath,
      format: "A4",
